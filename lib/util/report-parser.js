...
function getTimestamp(rawValue) {
  ...
  return new Date(1000 * rawValue).getTime();
}
...
function parseV4Report(reportBytes, reportTime, lastReportTime, lastReportTimestamp) {
  ...
  for (var index = 12; index + 9 <= reportBytes.length; index += 9) {
    ...
    report.data[entry.streamID][resolved_timestamp] = entry.value;
  }

  return report;
}

function parseReport(reportValue, reportTime, lastReportTime, lastReportTimestamp) {
  ...
  if (version === 4) {
    return parseV4Report(reportBytes, reportTime, lastReportTime, lastReportTimestamp);
  }
